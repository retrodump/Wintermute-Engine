...
void CScriptDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_NEW_SCRIPT, m_NewScript);
	//}}AFX_DATA_MAP
}
...
BOOL CScriptDlg::OnInitDialog() 
{
	...
	SetupButton(&m_AddScript,        "Attach existing script", IDB_OPEN);
	SetupButton(&m_NewScript,        "New script",             IDB_BCGBARRES_NEW);
	...
	if(m_ScriptNames.GetSize()==0) CreateScript();
	CenterWindow();
...
void CScriptDlg::OnAddScript() 
{
	...
	CString NewFile = GetRelativeFilename(m_Object->Game, "", "Script Files (*.dcs; *.script)|*.dcs;*.script", ((CSceneEditApp*)AfxGetApp())->m_LastDirScript, this, true);
	if(NewFile!=""){
...
void CScriptDlg::OnEditScript() 
{
	...
	((CSceneEditApp*)AfxGetApp())->EditFile(m_ScriptNames[sel], m_Object->Game, this);
}
...
//////////////////////////////////////////////////////////////////////////
void CScriptDlg::CreateScript()
{
	CTemplateDlg dlg;
	dlg.m_Subdir = "script";
	dlg.m_InitName = CString(m_Object->m_Name);
	dlg.Game = m_Object->Game;
	dlg.m_ProjectRoot = m_View->GetDocument()->m_ProjectRoot;
	dlg.m_Target = GetPath(m_View->GetDocument()->GetPathName()) + "scr";

	if(dlg.DoModal()==IDOK){
		int sel = m_Scripts.GetCurSel();
		CString NewFile = GetRelativePath(m_Object->Game, dlg.m_SelectedObject);
		if(NewFile!=""){
			for(int i=0; i<m_ScriptNames.GetSize(); i++){
				if(!m_ScriptNames[i].CompareNoCase(NewFile)){
					return;
