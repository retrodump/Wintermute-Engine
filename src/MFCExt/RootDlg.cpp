...
void CRootDlg::OnOK() 
{
	...
	if(m_Filename!=""){
		...
		if(result!=filename.Left(result.GetLength())){
			MessageBox(CString(LOC("/str1058/Folder")) + " '" + result + "' " + LOC("/str1059/is not a parent folder of file") + " '" + m_Filename + "'.", LOC("/str1002/Error"), MB_ICONERROR);
			return;
		}
	}
	...
	if(attr == 0xFFFFFFFF || (attr & FILE_ATTRIBUTE_DIRECTORY) == 0){
		if(MessageBox(CString(LOC("/str1060/Folder")) + " '" + result + "' " + LOC("/str1061/doesn't exist.\nDo you want to create it?"), NULL, MB_ICONQUESTION|MB_OKCANCEL)!=IDOK) return;

		result = MakePath(result);
		if(result==""){
			MessageBox(LOC("/str1062/The specified folder cannot be created."), LOC("/1002/Error"), MB_ICONERROR);
			return;
		}
	}
	...
	if(!finder.FindFile(result + "*.wpr")){
		if(MessageBox(LOC("/str1063/This doesn't appear to be a valid root folder (no project file was found).\n\nDo you want to continue anyway?"), LOC("/str1064/Warning"), MB_ICONWARNING|MB_YESNO)!=IDYES) return;
	}
