	...
	while ((cmd = parser.GetCommand ((char**)&Buffer, commands, (char**)&params)) > 0)
	{
		switch (cmd)
		{
			...
			case TOKEN_TEXT_ALIGN:
				if(_stricmp((char*)params, "center")==0) m_Align = TAL_CENTER;
				else if(_stricmp((char*)params, "right")==0) m_Align = TAL_RIGHT;
				else m_Align = TAL_LEFT;
			break;

			case TOKEN_VERTICAL_ALIGN:
				if(_stricmp((char*)params, "top")==0) m_VerticalAlign = VAL_TOP;
				else if(_stricmp((char*)params, "center")==0) m_VerticalAlign = VAL_CENTER;
...
HRESULT CAdResponseBox::Display()
{
	...
	if(m_Window)
	{
		OffsetRect(&rect, m_Window->m_PosX, m_Window->m_PosY);
		//m_Window->Display();
	}
...
HRESULT CAdResponseBox::Listen(CBScriptHolder* param1, DWORD param2)
{
	...
	{
		case UI_BUTTON:
			if(_stricmp(obj->m_Name, "prev")==0)
			{
				m_ScrollOffset--;
			}
			else if(_stricmp(obj->m_Name, "next")==0)
			{
				m_ScrollOffset++;
			}
			else if(_stricmp(obj->m_Name, "response")==0)
			{
