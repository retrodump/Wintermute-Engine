...
#include "PartEmitter.h"
	...
	#else
		#pragma comment(lib, "wme_base_d3d9.lib")
	#endif
#else
	#if _DEBUG
...
HRESULT CAdGame::RemoveObject(CAdObject *Object)
{
	...
	if(m_InventoryOwner==Object) m_InventoryOwner = NULL;
...
HRESULT CAdGame::ChangeScene(char *Filename, bool FadeIn)
{
	...
	{
		Game->m_PluginMgr->ApplyEvent(WME_EVENT_SCENE_SHUTDOWN, (IWmeObject*)m_Scene, NULL);
	...
	if(m_Scene)
	{
		...
		if(SUCCEEDED(ret))
		{
			...
			m_Scene->LoadState();
			Game->m_PluginMgr->ApplyEvent(WME_EVENT_SCENE_INIT, (IWmeObject*)m_Scene, NULL);
		}
...
HRESULT CAdGame::ScCallMethod(CScScript* Script, CScStack *Stack, CScStack *ThisStack, char *Name)
{
	...
	//////////////////////////////////////////////////////////////////////////
		...
		Stack->CorrectParams(1);
		if(!Game->m_UseD3D){
			Script->RuntimeError("Game.LoadActor3D: method is not available in compatibility mode");
			Stack->PushNULL();
			return S_OK;
		}
		...
		*/
		CAdObject3D* act = CAdActor3D::CreateFromFile(this, Stack->Pop()->GetString());
		if(act)
		{
			AddObject(act);
			Stack->PushNative(act, true);
		...
		if(!val->IsNULL()){
			for(int i=0; i<m_Inventories.GetSize(); i++){
				...
				for(int j=0; j<Inv->m_TakenItems.GetSize(); j++){
					...
					else if(stricmp(val->GetString(), Inv->m_TakenItems[j]->m_Name)==0){
						Stack->PushBool(true);
						return S_OK;
					}
				}
...
//////////////////////////////////////////////////////////////////////////
HRESULT CAdGame::LoadFile(char * Filename)
{
				...
				while (cmd > 0 && (cmd = parser.GetCommand ((char**)&params, commands, (char**)&params2)) > 0){
					switch(cmd){
						...
						case TOKEN_TALK_SKIP_BUTTON:
							if(stricmp((char*)params2, "right")==0) m_TalkSkipButton = TALK_SKIP_RIGHT;
							else if(stricmp((char*)params2, "both")==0) m_TalkSkipButton = TALK_SKIP_BOTH;
...
HRESULT CAdGame::Persist(CBPersistMgr *PersistMgr)
{
	...
	if(PersistMgr->CheckVersion(1, 9, 2))
	{
...
CAdSceneState* CAdGame::GetSceneState(char* Filename, bool Saving)
{
	...
	{
		if(stricmp(m_SceneStates[i]->m_Filename, FilenameCor)==0)
		{
			delete [] FilenameCor;
...
//////////////////////////////////////////////////////////////////////////
HRESULT CAdGame::StartDlgBranch(char *BranchName, char *ScriptName, char *EventName)
{
...
//////////////////////////////////////////////////////////////////////////
HRESULT CAdGame::EndDlgBranch(char *BranchName, char *ScriptName, char *EventName)
{
	char* Name = NULL;
	bool DeleteName = false;
...
HRESULT CAdGame::ClearBranchResponses(char *Name)
{
	for(int i=0; i<m_ResponsesBranch.GetSize(); i++){
		if(stricmp(Name, m_ResponsesBranch[i]->m_Context)==0){
...
bool CAdGame::BranchResponseUsed(int ID)
{
	...
	for(int i=0; i<m_ResponsesBranch.GetSize(); i++){
		if(m_ResponsesBranch[i]->m_ID==ID){
			if((Context==NULL && m_ResponsesBranch[i]->m_Context==NULL) || stricmp(Context, m_ResponsesBranch[i]->m_Context)==0) return true;
		}
	}
	return false;
}
...
bool CAdGame::GameResponseUsed(int ID)
{
	...
	for(int i=0; i<m_ResponsesGame.GetSize(); i++)
	{
		...
		{
			if((Context==NULL && RespContext->m_Context==NULL) || ((Context != NULL && RespContext->m_Context != NULL) && stricmp(Context, RespContext->m_Context)==0)) return true;
		}
	}
	return false;
}
...
HRESULT CAdGame::ResetResponse(int ID)
{
	...
	for(i=0; i<m_ResponsesGame.GetSize(); i++){
		if(m_ResponsesGame[i]->m_ID==ID){
			if((Context==NULL && m_ResponsesGame[i]->m_Context==NULL) || stricmp(Context, m_ResponsesGame[i]->m_Context)==0){
	...
	for(i=0; i<m_ResponsesBranch.GetSize(); i++){
		if(m_ResponsesBranch[i]->m_ID==ID){
			if((Context==NULL && m_ResponsesBranch[i]->m_Context==NULL) || stricmp(Context, m_ResponsesBranch[i]->m_Context)==0){
				delete m_ResponsesBranch[i];
				m_ResponsesBranch.RemoveAt(i);
				break;
			}
...
HRESULT CAdGame::DisplayContent(bool Update, bool DisplayAll)
{
	...
	{
		if(Update) m_VideoPlayer->Update();
		m_VideoPlayer->Display();
	}
	else if(m_TheoraPlayer)
	{
	...
	else
	{
		...
		// process plugin objects
		if(Update) Game->m_PluginMgr->ApplyEvent(WME_EVENT_UPDATE, NULL, NULL);
		...
		Game->m_PluginMgr->ApplyEvent(WME_EVENT_SCENE_DRAW_BEGIN, (IWmeObject*)m_Scene, NULL);
		m_Scene->Display();
		Game->m_PluginMgr->ApplyEvent(WME_EVENT_SCENE_DRAW_END, (IWmeObject*)m_Scene, NULL);
...
bool CAdGame::IsItemTaken(char* ItemName)
{
	...
	for(i=0; i<m_Inventories.GetSize(); i++){
		...
		for(int j=0; j<Inv->m_TakenItems.GetSize(); j++){
			if(stricmp(ItemName, Inv->m_TakenItems[j]->m_Name)==0){
				return true;
			}
		}
	}
...
{
	for(int i=0; i<m_Items.GetSize(); i++){
		if(_stricmp(m_Items[i]->m_Name, Name)==0) return m_Items[i];
	}
	return NULL;
}
...
//////////////////////////////////////////////////////////////////////////
HRESULT CAdGame::RenderShadowGeometry()
{
	if(m_Scene && m_Scene->m_Geom) return m_Scene->m_Geom->RenderShadowGeometry();
	else return S_OK;
}
...
CBObject* CAdGame::GetNextAccessObject(CBObject* CurrObject)
{
	...
	if(!Ret)
	{
		if(m_ResponseBox && m_StateEx==GAME_WAITING_RESPONSE) return m_ResponseBox->GetNextAccessObject(CurrObject);
		if(m_Scene) return m_Scene->GetNextAccessObject(CurrObject);
	}
	return Ret;
}
...
CBObject* CAdGame::GetPrevAccessObject(CBObject* CurrObject)
{
	...
	if(!Ret)
	{
		if(m_ResponseBox && m_StateEx==GAME_WAITING_RESPONSE) return m_ResponseBox->GetPrevAccessObject(CurrObject);
		if(m_Scene) return m_Scene->GetPrevAccessObject(CurrObject);
	}
	return Ret;
}
...
HRESULT CAdGame::OnMouseLeftDown() 
{
	...
	SetCapture(m_Renderer->m_Window);
...
//////////////////////////////////////////////////////////////////////////
TShadowType CAdGame::GetMaxShadowType(CBObject* Object)
{
	TShadowType Ret = CBGame::GetMaxShadowType(Object);
	return min(Ret, m_Scene->m_MaxShadowType);
}
...
HRESULT CAdGame::GetLayerSize(int* LayerWidth, int* LayerHeight, RECT* Viewport, bool* CustomViewport)
{
	if(m_Scene && m_Scene->m_MainLayer)
	{
		...
		m_Scene->GetViewportOffset(&PortX, &PortY);
		m_Scene->GetViewportSize(&PortWidth, &PortHeight);
		*CustomViewport = m_SceneViewport || m_Scene->m_Viewport;

		CBPlatform::SetRect(Viewport, PortX, PortY, PortX + PortWidth, PortY + PortHeight);

		if(!m_Scene->m_Scroll3DCompatibility)
		{
			*LayerWidth = m_Scene->m_MainLayer->m_Width;
			*LayerHeight = m_Scene->m_MainLayer->m_Height;
		}
		else
		{
			// backward compatibility hack
			// WME pre-1.7 expects the camera to only view the top-left part of the scene
			*LayerWidth = Game->m_Renderer->m_Width;
			*LayerHeight = Game->m_Renderer->m_Height;
...
HRESULT CAdGame::GetFogParams(bool* FogEnabled, DWORD* FogColor, float* Start, float* End)
{
	if(m_Scene)
	{
		...
		*FogColor = m_Scene->m_FogColor;
		*Start = m_Scene->m_FogStart;
		*End = m_Scene->m_FogEnd;
		return S_OK;
	}
