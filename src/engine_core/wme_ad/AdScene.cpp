...
//m_Mirror = new CAdMirror(D3DXVECTOR3(270,190,220), D3DXVECTOR3(400,190,220), D3DXVECTOR3(400,0,220), D3DXVECTOR3(270,0,220));
...
void CAdScene::SetDefaults()
{
	...
	m_2DPathfinding = false;
	m_MaxShadowType = SHADOW_FLAT;
	
	m_Scroll3DCompatibility = false;
	m_AmbientLightColor = 0x00000000;

	m_FogEnabled = false;
	m_FogColor = 0x00FFFFFF;
	m_FogStart = 0.0f;
	m_FogEnd = 0.0f;
}
...
HRESULT CAdScene::InitLoop()
{
	...
	while(!m_PFReady && timeGetTime() - start <= m_PFMaxTime)
	{
		PathFinderStep();
		num_steps++;
	}
...
#else
	DWORD start = Game->m_CurrentTime;
	while(!m_PFReady && timeGetTime() - start <= m_PFMaxTime) PathFinderStep();
#endif

	if(m_Geom) return m_Geom->InitLoop();
	else return S_OK;
}
...
TOKEN_DEF_START
	...
	TOKEN_DEF (WAYPOINT_HEIGHT)
	TOKEN_DEF (NEAR_CLIPPING_PLANE)
	TOKEN_DEF (FAR_CLIPPING_PLANE)
	TOKEN_DEF (2D_PATHFINDING)
	TOKEN_DEF (MAX_SHADOW_TYPE)
	TOKEN_DEF (SCROLL_3D_COMPATIBILITY)
...
HRESULT CAdScene::LoadBuffer(BYTE * Buffer, bool Complete)
{
	TOKEN_TABLE_START(commands)
		...
		TOKEN_TABLE (WAYPOINT_HEIGHT)
		TOKEN_TABLE (NEAR_CLIPPING_PLANE)
		TOKEN_TABLE (FAR_CLIPPING_PLANE)
		TOKEN_TABLE (2D_PATHFINDING)
		TOKEN_TABLE (MAX_SHADOW_TYPE)
		TOKEN_TABLE (SCROLL_3D_COMPATIBILITY)
	...
	while ((cmd = parser.GetCommand ((char**)&Buffer, commands, (char**)&params)) > 0)
	{
		switch (cmd)
		{
			...
			case TOKEN_GEOMETRY:
				SAFE_DELETE(m_Geom);
				if(!Game->m_UseD3D) break;
				m_Geom = new CAdSceneGeometry(Game);
				if(!m_Geom || FAILED(m_Geom->LoadFile((char*)params)))
				{
					SAFE_DELETE(m_Geom);
					cmd = PARSERR_GENERIC;
				}
			...
			case TOKEN_EDITOR_SHOW_GEOMETRY:
				parser.ScanStr((char*)params, "%b", &m_ShowGeometry);
				if(!Game->m_EditorMode) m_ShowGeometry = false;
			break;
			...
			case TOKEN_EDITOR_RESOLUTION_WIDTH:
				parser.ScanStr((char*)params, "%d", &m_EditorResolutionWidth);
			break;

			case TOKEN_EDITOR_RESOLUTION_HEIGHT:
				parser.ScanStr((char*)params, "%d", &m_EditorResolutionHeight);
			break;

			case TOKEN_FOV_OVERRIDE:
				parser.ScanStr((char*)params, "%f", &m_FOV);
			break;

			case TOKEN_WAYPOINT_HEIGHT:
				parser.ScanStr((char*)params, "%f", &WaypointHeight);
			break;

			case TOKEN_NEAR_CLIPPING_PLANE:
				parser.ScanStr((char*)params, "%f", &m_NearClipPlane);
			break;

			case TOKEN_FAR_CLIPPING_PLANE:
				parser.ScanStr((char*)params, "%f", &m_FarClipPlane);
			break;

			case TOKEN_2D_PATHFINDING:
				parser.ScanStr((char*)params, "%b", &m_2DPathfinding);
			break;

			case TOKEN_MAX_SHADOW_TYPE:
			{
				int MaxShadowType;
				parser.ScanStr((char*)params, "%d", &MaxShadowType);
				SetMaxShadowType((TShadowType)MaxShadowType);
			}
			break;

			case TOKEN_SCROLL_3D_COMPATIBILITY:
				parser.ScanStr((char*)params, "%b", &m_Scroll3DCompatibility);
			break;

			case TOKEN_AMBIENT_LIGHT_COLOR:
				parser.ScanStr((char*)params, "%d,%d,%d", &ar, &ag, &ab);
				m_AmbientLightColor = DRGBA(ar, ag, ab, 255);
			break;
		}
	}
	...
	if(m_Geom && camera[0]!='\0')
	{
		m_Geom->SetActiveCamera(camera, m_FOV, m_NearClipPlane, m_FarClipPlane);
	}
	...
	if(m_Geom)
	{
		...
		{
			m_Geom->m_WaypointHeight = WaypointHeight;
			m_Geom->DropWaypoints();
		}

		if(m_Geom->m_ActiveCamera >= 0 && m_Geom->m_ActiveCamera < m_Geom->m_Cameras.GetSize())
		{
			Game->m_Renderer->Setup3D(m_Geom->m_Cameras[m_Geom->m_ActiveCamera]);
			Game->m_Renderer->SetScreenViewport();
			m_Geom->Render(false);
		}
	}
	...
	if(m_MainLayer)
	{
		if(m_EditorResolutionWidth<=0) m_EditorResolutionWidth = m_MainLayer->m_Width;
		if(m_EditorResolutionHeight<=0) m_EditorResolutionHeight = m_MainLayer->m_Height;
	}
...
HRESULT CAdScene::TraverseNodes(bool Update)
{
	...
	for(j=0; j<m_Layers.GetSize(); j++)
	{
		...
		if(!Update && m_Geom && m_Layers[j]->m_Main)
		{
			Game->GetOffset(&MainOffsetX, &MainOffsetY);

			TShadowType ShadowType = Game->GetMaxShadowType();
	...
	if(!Update && m_Geom)
	{
		CAdMirror* Mirror = new CAdMirror(Game, D3DXVECTOR3(270,190,220), D3DXVECTOR3(400,190,220), D3DXVECTOR3(400,0,220), D3DXVECTOR3(270,0,220));
		//CAdMirror* Mirror = new CAdMirror(Game, D3DXVECTOR3(-490,0,240), D3DXVECTOR3(1000,0,240), D3DXVECTOR3(1000,0,-800), D3DXVECTOR3(-490,0,-800));
		//CAdMirror* Mirror = new CAdMirror(Game, D3DXVECTOR3(-490,50,240), D3DXVECTOR3(1000,50,240), D3DXVECTOR3(1000,0,-800), D3DXVECTOR3(-490,0,-800));
		Mirror->UpdateMatrices(m_Geom->m_Cameras[m_Geom->m_ActiveCamera]);

		Display3DContent(&Mirror->m_MatView, &Mirror->m_MatProj);
		//Display3DContent(&Mirror->m_MatView, NULL);
	...
	*/
	...
	if(!Update && m_Geom)
	{
		Game->SetOffset(MainOffsetX, MainOffsetY);
		m_Geom->Render(m_ShowGeometry);
	}
...
//////////////////////////////////////////////////////////////////////////
HRESULT CAdScene::Display3DContent(D3DXMATRIX* ViewMat, D3DXMATRIX* ProjMat)
{
	if(!m_Geom) return E_FAIL;

	Game->m_Renderer->Setup3DCustom(ViewMat, ProjMat);
	...
	for(int j=0; j<m_Layers.GetSize(); j++)
	{
		...
		// for each node
		for(int k=0; k<m_Layers[j]->m_Nodes.GetSize(); k++)
		{
			CAdSceneNode* Node = m_Layers[j]->m_Nodes[k];
			if(Node->m_Type == OBJECT_REGION)
			{
				if(Node->m_Region->m_Blocked) continue;
				if(Node->m_Region->m_Decoration) continue;

				DisplayRegionContent(Node->m_Region, true);
			}
		} // each node
		...
		if(m_Layers[j]->m_Main)
		{
			DisplayRegionContent(NULL, true);
		}
	} // each layer
...
HRESULT CAdScene::UpdateFreeObjects()
{
	...
	for(i=0; i<AdGame->m_Objects.GetSize(); i++)
	{
		...
		if(AdGame->m_Objects[i]->m_Is3D && m_Geom)
		{
			if(m_Geom->m_ActiveCamera >= 0 && m_Geom->m_ActiveCamera < m_Geom->m_Cameras.GetSize())
			{
				Game->m_Renderer->Setup3D(m_Geom->m_Cameras[m_Geom->m_ActiveCamera], !Is3DSet);
				Is3DSet = true;
			}
		}
...
HRESULT CAdScene::ScCallMethod(CScScript* Script, CScStack *Stack, CScStack *ThisStack, char *Name)
{
	...
	else if(strcmp(Name, "LoadActor3D")==0)
	{
		...
		/*
		CAdActor3D* act = new CAdActor3D(Game);
		if(act && SUCCEEDED(act->LoadFile(Stack->Pop()->GetString())))
		{
			AddObject(act);
			Stack->PushNative(act, true);
		...
		*/
		...
		CAdObject3D* act = CAdActor3D::CreateFromFile(Game, Stack->Pop()->GetString());
		if(act)
		{
			AddObject(act);
			Stack->PushNative(act, true);
	...
	else if(strcmp(Name, "GetLayer")==0)
	{
		...
		else
		{
			...
			{
				if(stricmp(LayerName, m_Layers[i]->m_Name)==0)
				{
	...
	else if(strcmp(Name, "GetFreeNode")==0)
	{
		...
		else
		{
			...
			{
				if(m_Objects[i] && m_Objects[i]->m_Name && stricmp(m_Objects[i]->m_Name, Name)==0)
				{
					Ret = m_Objects[i];
					break;
				}
	...
	else if(strcmp(Name, "EnableNode3D")==0)
	{
		...
		{
			HRESULT Ret = m_Geom->EnableNode(Name, true);
	...
	else if(strcmp(Name, "DisableNode3D")==0)
	{
		...
		if(!m_Geom)
		{
			Script->RuntimeError("Scene.DisableNode3D: Scene doesn't contain any geometry.");
			Stack->PushBool(false);
		}
		else
		{
			HRESULT Ret = m_Geom->EnableNode(Name, false);
			Stack->PushBool(SUCCEEDED(Ret));
		}
		return S_OK;
	}
	...
	else if(strcmp(Name, "IsNode3DEnabled")==0)
	{
		...
		if(m_Geom)
		{
			Stack->PushBool(m_Geom->IsNodeEnabled(Name));
		}
	...
	else if(strcmp(Name, "EnableLight")==0)
	{
		...
		{
			HRESULT Ret = m_Geom->EnableLight(Name, true);
			Stack->PushBool(SUCCEEDED(Ret));
		}
		return S_OK;
	}
	...
	else if(strcmp(Name, "DisableLight")==0)
	{
		...
		{
			HRESULT Ret = m_Geom->EnableLight(Name, false);
			Stack->PushBool(SUCCEEDED(Ret));
		}
		return S_OK;
	}
	...
	else if(strcmp(Name, "IsLightEnabled")==0)
	{
		...
		if(m_Geom)
		{
			Stack->PushBool(m_Geom->IsLightEnabled(Name));
		}
		else Stack->PushBool(false);
	...
	else if(strcmp(Name, "GetLightName")==0)
	{
		...
		if(m_Geom)
		{
			if(Index >= 0 && Index < m_Geom->m_Lights.GetSize())
				Stack->PushString(m_Geom->m_Lights[Index]->m_Name);
			else
				Stack->PushNULL();
		}
		else Stack->PushNULL();
	...
	else if(strcmp(Name, "SetLightColor")==0)
	{
		Stack->CorrectParams(2);
		char* Name = Stack->Pop()->GetString();
		DWORD Color = (DWORD)Stack->Pop()->GetInt();
		if(!m_Geom)
		{
		...
		{
			HRESULT Ret = m_Geom->SetLightColor(Name, Color);
			Stack->PushBool(SUCCEEDED(Ret));
		}
		return S_OK;
	}
	...
	else if(strcmp(Name, "GetLightColor")==0)
	{
		...
		{
			Stack->PushInt(m_Geom->GetLightColor(Name));
		}
		return S_OK;
	}
	...
	else if(strcmp(Name, "GetLightPosition")==0)
	{
		...
		{
			D3DXVECTOR3 pos = m_Geom->GetLightPos(Name);
	...
	else if(strcmp(Name, "SetActiveCamera")==0)
	{
		...
		{
			HRESULT Ret = m_Geom->SetActiveCamera(Name, m_FOV, m_NearClipPlane, m_FarClipPlane);
			if(FAILED(Ret)) Script->RuntimeError("Scene.SetActiveCamera failed.");
			Stack->PushBool(SUCCEEDED(Ret));
		}
		return S_OK;
	}
	...
	else if(strcmp(Name, "EnableFog")==0)
	{
		Stack->CorrectParams(3);
		m_FogEnabled = true;
		m_FogColor = Stack->Pop()->GetInt();
		m_FogStart = Stack->Pop()->GetFloat();
		m_FogEnd = Stack->Pop()->GetFloat();
...
CScValue* CAdScene::ScGetProperty(char *Name)
{
	...
	{
		m_ScValue->SetBool(m_ShowGeometry);
		return m_ScValue;
	}
	...
	{
		if(m_Geom && m_Geom->m_Filename) m_ScValue->SetString(m_Geom->m_Filename);
		else m_ScValue->SetNULL();
		return m_ScValue;
	}
	...
	{
		if(m_Geom) m_ScValue->SetFloat(m_Geom->m_WaypointHeight);
		else m_ScValue->SetFloat(0.0f);
		return m_ScValue;
	}
	...
	{
		m_ScValue->SetInt(m_AmbientLightColor);
		return m_ScValue;
	}
	...
	{
		if(!m_Geom) m_ScValue->SetInt(0);
		else m_ScValue->SetInt(m_Geom->m_Lights.GetSize());
		return m_ScValue;
	}
...
HRESULT CAdScene::ScSetProperty(char *Name, CScValue *Value)
{
	...
	else if(strcmp(Name, "WaypointsHeight")==0)
	{
		if(m_Geom)
		{
			m_Geom->m_WaypointHeight = Value->GetFloat();
			m_Geom->DropWaypoints();
		}
		return S_OK;
	}
...
HRESULT CAdScene::SaveAsText(CBDynBuffer *Buffer, int Indent)
{
	...
	if(m_Geom)
	{
		if(m_Geom->m_Filename) Buffer->PutTextIndent(Indent+2, "GEOMETRY=\"%s\"\n", m_Geom->m_Filename);
		if(m_Geom->m_ActiveCamera >= 0 && m_Geom->m_ActiveCamera < m_Geom->m_Cameras.GetSize())
		{
			Buffer->PutTextIndent(Indent+2, "CAMERA=\"%s\"\n", m_Geom->m_Cameras[m_Geom->m_ActiveCamera]->m_Name);
		}
...
HRESULT CAdScene::Persist(CBPersistMgr* PersistMgr)
{
	...
	PersistMgr->Transfer(TMEMBER(m_Geom));
	...
	PersistMgr->Transfer(TMEMBER(m_ShowGeometry));
	...
	PersistMgr->Transfer(TMEMBER(m_FOV));
	PersistMgr->Transfer(TMEMBER(m_NearClipPlane));
	PersistMgr->Transfer(TMEMBER(m_FarClipPlane));

	PersistMgr->Transfer(TMEMBER(m_2DPathfinding));
	PersistMgr->Transfer(TMEMBER_INT(m_MaxShadowType));

	PersistMgr->Transfer(TMEMBER(m_Scroll3DCompatibility));

	if(PersistMgr->CheckVersion(1, 7, 2))
	{
		PersistMgr->Transfer(TMEMBER(m_AmbientLightColor));
	}
	else m_AmbientLightColor = 0x00000000;

	if(PersistMgr->CheckVersion(1, 7, 3))
	{
		PersistMgr->Transfer(TMEMBER(m_FogEnabled));
		PersistMgr->Transfer(TMEMBER(m_FogColor));
		PersistMgr->Transfer(TMEMBER(m_FogStart));
		PersistMgr->Transfer(TMEMBER(m_FogEnd));
	}
	else
	{
		m_FogEnabled = false;
...
{
	if(m_Geom && m_Geom->m_ActiveCamera >= 0 && m_Geom->m_ActiveCamera < m_Geom->m_Cameras.GetSize())
	{
		m_Geom->SetActiveCamera(m_Geom->m_ActiveCamera, m_FOV, m_NearClipPlane, m_FarClipPlane);
	}
...
CBObject* CAdScene::GetNodeByName(char *Name)
{
	...
	for(i=0; i<m_Layers.GetSize(); i++)
	{
		...
		for(int j=0; j<layer->m_Nodes.GetSize(); j++)
		{
			...
			if( (node->m_Type==OBJECT_ENTITY && !stricmp(Name, node->m_Entity->m_Name))||
				(node->m_Type==OBJECT_REGION && !stricmp(Name, node->m_Region->m_Name)))
			{
				switch(node->m_Type)
				{
					case OBJECT_ENTITY:
	...
	for(i=0; i<m_Objects.GetSize(); i++)
	{
		if(m_Objects[i]->m_Type==OBJECT_ENTITY && !stricmp(Name, m_Objects[i]->m_Name))
		{
			return m_Objects[i];
		}
	}
	...
	for(i=0; i<m_WaypointGroups.GetSize(); i++)
	{
		if(!stricmp(Name, m_WaypointGroups[i]->m_Name))
		{
...
//////////////////////////////////////////////////////////////////////////
C3DLight* CAdScene::GetActiveLight()
{
	if(m_Geom && m_Geom->m_ActiveLight>=0 && m_Geom->m_ActiveLight<m_Geom->m_Lights.GetSize())
		return m_Geom->m_Lights[m_Geom->m_ActiveLight];
	else
		return NULL;
}
...
//////////////////////////////////////////////////////////////////////////
HRESULT CAdScene::SetMaxShadowType(TShadowType MaxShadowType)
{
...
//////////////////////////////////////////////////////////////////////////
void CAdScene::OnLayerResized(CAdLayer* layer)
{
	if (layer != NULL && layer == m_MainLayer)
	{
		m_Width = layer->m_Width;
		m_Height = layer->m_Height;
	}
