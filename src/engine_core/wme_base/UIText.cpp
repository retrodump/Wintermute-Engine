	...
	while (cmd > 0 && (cmd = parser.GetCommand ((char**)&Buffer, commands, (char**)&params)) > 0)
	{
		switch (cmd)
		{
			...
			case TOKEN_TEXT_ALIGN:
				if(_stricmp((char*)params, "left")==0) m_TextAlign = TAL_LEFT;
				else if(_stricmp((char*)params, "right")==0) m_TextAlign = TAL_RIGHT;
				else m_TextAlign = TAL_CENTER;
			break;

			case TOKEN_VERTICAL_ALIGN:
				if(_stricmp((char*)params, "top")==0) m_VerticalAlign = VAL_TOP;
				else if(_stricmp((char*)params, "bottom")==0) m_VerticalAlign = VAL_BOTTOM;
