...
HRESULT CBSprite::LoadFile(char* Filename, int LifeTime, TSpriteCacheType CacheType)
{
	...
	if(strnicmp(Filename, "savegame:", 9)==0 || _stricmp(ext, ".bmp")==0 || _stricmp(ext, ".tga")==0 || _stricmp(ext, ".png")==0 || _stricmp(ext, ".jpg")==0)
	{
		...
		{
			SetRect(&subframe->m_Rect, 0, 0, subframe->m_Surface->GetWidth(), subframe->m_Surface->GetHeight());
