...
HRESULT CPartEmitter::AddSprite(char* Filename)
{
	...
	for(int i=0; i < m_Sprites.GetSize(); i++)
	{
		if(stricmp(Filename, m_Sprites[i])==0) return S_OK;
	}
...
HRESULT CPartEmitter::RemoveSprite(char* Filename)
{
	for(int i=0; i < m_Sprites.GetSize(); i++)
	{
		if(stricmp(Filename, m_Sprites[i])==0)
		{
			delete [] m_Sprites[i];
...
HRESULT CPartEmitter::InitParticle(CPartParticle* Particle, DWORD CurrentTime, DWORD TimerDelta)
{
	...
	D3DXVECTOR2 VecPos = D3DXVECTOR2((float)PosX, (float)PosY);
	D3DXVECTOR2 VecVel = D3DXVECTOR2(0, Velocity);

	D3DXMATRIX MatRot;
	D3DXMatrixRotationZ(&MatRot, DegToRad(CBUtils::NormalizeAngle(Angle - 180)));
	D3DXVec2TransformCoord(&VecVel, &VecVel, &MatRot);
...
{
	SetRect(&m_Border, X, Y, X + Width, Y + Height);
...
CPartForce* CPartEmitter::AddForceByName(char* Name)
{
	...
	for(int i=0; i<m_Forces.GetSize(); i++)
	{
		if(stricmp(Name, m_Forces[i]->m_Name)==0)
		{
			Force = m_Forces[i];
			break;
		}
