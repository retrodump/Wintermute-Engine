...
HRESULT CUIWindow::Display(int OffsetX, int OffsetY)
{
	...
	if(!IsRectEmpty(&m_TitleRect) && font && m_Text)
	{
	...
	while (cmd >= PARSERR_TOKENNOTFOUND && (cmd = parser.GetCommand ((char**)&Buffer, commands, (char**)&params)) >= PARSERR_TOKENNOTFOUND)
	{
		switch (cmd)
		{
			...
			case TOKEN_TITLE_ALIGN:
				if(_stricmp((char*)params, "left")==0) m_TitleAlign = TAL_LEFT;
				else if(_stricmp((char*)params, "right")==0) m_TitleAlign = TAL_RIGHT;
...
HRESULT CUIWindow::EnableWidget(char *Name, bool Enable)
{
	for(int i=0; i<m_Widgets.GetSize(); i++)
	{
		if(_stricmp(m_Widgets[i]->m_Name, Name)==0) m_Widgets[i]->m_Disable = !Enable;
	}
	return S_OK;
}
...
HRESULT CUIWindow::ShowWidget(char *Name, bool Visible)
{
	for(int i=0; i<m_Widgets.GetSize(); i++)
	{
		if(_stricmp(m_Widgets[i]->m_Name, Name)==0) m_Widgets[i]->m_Visible = Visible;
	}
	return S_OK;
}
...
HRESULT CUIWindow::ScCallMethod(CScScript* Script, CScStack *Stack, CScStack *ThisStack, char *Name)
{
	...
	if(strcmp(Name, "GetWidget")==0 || strcmp(Name, "GetControl")==0)
	{
		...
		else
		{
			for(int i=0; i<m_Widgets.GetSize(); i++)
			{
				if(stricmp(m_Widgets[i]->m_Name, val->GetString())==0)
				{
...
//////////////////////////////////////////////////////////////////////////
bool CUIWindow::HandleKeypress(bool Printable, DWORD CharCode, DWORD KeyData)
{
	if(!Printable && CharCode==9)
	{
		return SUCCEEDED(MoveFocus((GetAsyncKeyState(VK_SHIFT) & 0x8000)==0));
	}
	else
	{
		if(m_FocusedWidget) return m_FocusedWidget->HandleKeypress(Printable, CharCode, KeyData);
		else return false;
	}
}
...
HRESULT CUIWindow::HandleMouse(TMouseEvent Event, TMouseButton Button)
{
	...
	if(!IsRectEmpty(&m_DragRect))
	{
		...
		if(Event==MOUSE_CLICK && Button==MOUSE_BUTTON_LEFT)
		{
			...
			OffsetRect(&DragRect, m_PosX+OffsetX, m_PosY+OffsetY);

			if(PtInRect(&DragRect, Game->m_MousePos))
			{
				m_DragFrom.x = Game->m_MousePos.x;
				m_DragFrom.y = Game->m_MousePos.y;
				m_Dragging = true;
...
HRESULT CUIWindow::Listen(CBScriptHolder* param1, DWORD param2)
{
	...
	{
		case UI_BUTTON:
			if(_stricmp(obj->m_Name, "close")==0) Close();
