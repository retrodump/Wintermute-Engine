...
//////////////////////////////////////////////////////////////////////////
HRESULT CBStringTable::LocalizeWindow(HWND Window)
{
	if(!Window) return E_FAIL;

	LocWinCallback(Window, (LPARAM)this);
...
BOOL CBStringTable::LocWinCallback(HWND hwnd, LPARAM lParam)
{
	...
	CBStringTable* StringTable = (CBStringTable*)lParam;
	if(!StringTable) return FALSE;

	char WinText[WIN_TEXT_BUFFER_LENGTH];
	::GetWindowText(hwnd, WinText, WIN_TEXT_BUFFER_LENGTH);
	const char* Str = StringTable->ExpandStatic(WinText);

	//::SetWindowText(hwnd, StringTable->ExpandStatic(WinText));
	CBTextUtils::SetWindowText(StringTable->Game->m_TextEncoding, hwnd, (BYTE*)Str);
