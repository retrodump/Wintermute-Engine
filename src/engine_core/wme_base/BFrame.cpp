...
HRESULT CBFrame::Draw(int X, int Y, CBObject* Register, float ZoomX, float ZoomY, bool Precise, DWORD Alpha, bool AllFrames, float Rotate, TSpriteBlendMode BlendMode)
{
	...
	for(int i=0; i<m_Subframes.GetSize(); i++)
	{
		...
		if(!AllFrames)
		{
			if((m_Subframes[i]->m_2DOnly && Game->m_UseD3D) || (m_Subframes[i]->m_3DOnly && !Game->m_UseD3D))
				continue;
		}
...
HRESULT CBFrame::LoadBuffer(BYTE * Buffer, int LifeTime, bool KeepLoaded)
{
	...
	if(IsRectEmpty(&rect)) sub->SetDefaultRect();
...
HRESULT CBFrame::ScCallMethod(CScScript* Script, CScStack *Stack, CScStack *ThisStack, char *Name)
{
	...
	else if(strcmp(Name, "AddEvent")==0)
	{
		...
		for(int i=0; i<m_ApplyEvent.GetSize(); i++)
		{
			if(stricmp(m_ApplyEvent[i], Event)==0)
			{
				Stack->PushNULL();
				return S_OK;
			}
