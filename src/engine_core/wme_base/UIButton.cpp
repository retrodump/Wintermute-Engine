	...
	while (cmd>0 && (cmd = parser.GetCommand ((char**)&Buffer, commands, (char**)&params)) > 0)
	{
		switch (cmd)
		{
			...
			case TOKEN_TEXT_ALIGN:
				if(_stricmp((char*)params, "left")==0) m_Align = TAL_LEFT;
...
{	
	...
	//RECT rect;
	//SetRect(&rect, OffsetX + m_PosX, OffsetY + m_PosY, OffsetX+m_PosX+m_Width, OffsetY+m_PosY+m_Height);
	//m_Hover = (!m_Disable && PtInRect(&rect, Game->m_MousePos)!=FALSE);
	...
	if((m_Press && m_Hover && !Game->m_MouseLeftDown) ||
		m_OneTimePress && timeGetTime() - m_OneTimePressTime >= 100) Press();
...
HRESULT CUIButton::ScCallMethod(CScScript* Script, CScStack *Stack, CScStack *ThisStack, char *Name)
{
	...
	//////////////////////////////////////////////////////////////////////////
		...
		if(m_Visible && !m_Disable){
			m_OneTimePress = true;
			m_OneTimePressTime = timeGetTime();
		}
		Stack->PushNULL();
