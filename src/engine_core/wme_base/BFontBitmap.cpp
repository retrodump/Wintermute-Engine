...
CBFontBitmap::CBFontBitmap(CBGame* inGame):CBFont(inGame)
{
	...
	m_Sprite = NULL;
	m_WidthsFrame = 0;
	ZeroMemory(m_Widths, NUM_CHARACTERS);
...
//////////////////////////////////////////////////////////////////////
int CBFontBitmap::GetTextWidth(BYTE* text, int MaxLenght)
{
	bool NeedsDelete = false;
	if(Game->m_TextEncoding==TEXT_UTF8)
	{
		text = (BYTE*)CBTextUtils::Utf8ToAnsi((char*)text);
		NeedsDelete = true;
	}
	...
	ch = text[index];
	while(ch!='\0' && (MaxLenght<0 || index<MaxLenght))
	{
		TextWidth += GetCharWidth(ch);
		index++;
		ch = text[index];
	}

	if(NeedsDelete) delete [] text;
...
int CBFontBitmap::TextHeightDraw(BYTE* text, int x, int y, int width, TTextAlign align, bool draw, int max_height, int MaxLenght)
{
	...
	while(!done)
	{
		...
		if(text[index]==' ' && (max_height<0 || max_height/m_TileHeight>1))
		{
			end = index-1;
		...
		if(LineLength+GetCharWidth(text[index])>width && last_end==end)
		{
			end = index-1;
			RealLength = LineLength;
			new_line = true;
		...
		if(text[index+1]=='\0' || (MaxLenght>=0 && index==MaxLenght-1))
		{
			done = true;			
			if(!new_line)
			{
				end = index;
				LineLength+=GetCharWidth(text[index]);
				RealLength = LineLength;
			}
		}
		else LineLength+=GetCharWidth(text[index]);
		...
		if((LineLength>width) || done || new_line)
		{
			...
			{
				if(draw) DrawChar(text[i], StartX, y);
				StartX+=GetCharWidth(text[i]);
			}
			y+=m_TileHeight;
			last_end = end;
			if(long_line) end--;
...
void CBFontBitmap::DrawChar(BYTE c, int x, int y)
{
	...
	SetRect(&rect, col*m_TileWidth, row*m_TileHeight, col*m_TileWidth+TileWidth, (row+1)*m_TileHeight);
	bool Handled = false;
	if(m_Sprite)
	{
